...
def sort_yaml_file(input_file: str):
    ...
    # Copy data in sorted order, preserving comments
    for key in sorted_keys:
        sorted_data[key] = data[key]

        # Keep comments associated for current key
        if hasattr(data, 'ca') and data.ca.items:
            if key in data.ca.items:
                if not hasattr(sorted_data, 'ca'):
                    sorted_data.ca = data.ca.__class__()
                if not hasattr(sorted_data.ca, 'items'):
                    sorted_data.ca.items = {}
